...
class Panel:
    ...
    sub: str
    card: bool = False     # a cropped card rather than a full screen
...
def compose(panel: Panel, w: int, h: int) -> Image.Image:
    ...
    if panel.card:
        # A cropped card: show it as a floating card, larger, no bezel.
        art_w = int(w * 0.78)
        art = rounded(shot.resize((art_w, int(art_w * shot.height / shot.width)), Image.LANCZOS), int(art_w * 0.06))
    else:
