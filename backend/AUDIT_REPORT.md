...
| `subscriptions` | id, **user_id UNIQUE**, **plan_id**(basic), stripe_subscription_id, stripe_price_id, **status**(active) ∈ {active, canceled, past_due, trialing}, current_period_*, **daily_swipe_limit**(10), swipes_used_today(0), swipes_reset_at(CURRENT_DATE), timestamps | + CHECK plan_id (mig 008) |
