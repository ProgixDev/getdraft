    ...
    const { data, error } = await supabase
      ...
      .select(
        'id, guardian_user_id, relationship, status, created_at, decided_at, ' +
          'guardian:users!guardian_links_guardian_user_id_fkey ( id, name, email, profile_photo_url )',
      )
    ...
    const { data } = await supabase
      ...
      .select(
        'id, athlete_user_id, relationship, status, video_storage_path, ' +
          ...
          'athlete:users!guardian_links_athlete_user_id_fkey ( id, name, profile_photo_url )',
      )
      .eq('guardian_user_id', guardianUserId)
      .order('created_at', { ascending: false })
      .limit(1)
    ...
    let query = supabase
      ...
      .select(
        'id, athlete_user_id, guardian_user_id, relationship, questionnaire, ' +
          ...
          'athlete:users!guardian_links_athlete_user_id_fkey ( id, name, email, profile_photo_url ), ' +
          'guardian:users!guardian_links_guardian_user_id_fkey ( id, name, email, profile_photo_url )',
      )
      .order('created_at', { ascending: false });
