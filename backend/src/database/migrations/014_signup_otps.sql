-- 014: Signup OTPs for the new email + phone signup flow
-- (ported from reference 010_signup_otps.sql)
--
-- The backend owns the verification step (SMTP for email; Twilio Verify
-- for phone). A Supabase auth user is created ONLY after the full signup
-- flow completes (`/auth/complete-signup`). This table holds the
-- in-flight OTPs.
