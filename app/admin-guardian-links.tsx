...
export default function AdminGuardianLinksScreen() {
  ...
  const decide = useCallback(async (link: LinkWithVideo, decision: 'approved' | 'declined') => {
    if (busyId) return;
    const noteRef: { value: string } = { value: '' };
    Alert.prompt(
          ...
          onPress: async (notes?: string) => {
            ...
            try {
              if (decision === 'approved') {
                await guardianLinksService.adminApprove(link.id, notes || undefined);
              } else {
                await guardianLinksService.adminDecline(link.id, notes || undefined);
              }
              await refresh(tab);
            } catch (err: any) {
    ...
    <View style={styles.card}>
      <View style={styles.row}>
        <View style={styles.avatarPair}>
          <Avatar uri={props.link.guardian?.profile_photo_url} />
          <Ionicons name="link" size={16} color={theme.textSecondary} />
          <Avatar uri={props.link.athlete?.profile_photo_url} />
        </View>
