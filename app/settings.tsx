...
export default function SettingsScreen() {
  ...
  const [messageNotifications, setMessageNotifications] = useState(PREF_DEFAULTS.messageNotifications);
  const [recruiterActivity, setRecruiterActivity] = useState(PREF_DEFAULTS.recruiterActivity);
  const [profileVisible, setProfileVisible] = useState(PREF_DEFAULTS.profileVisible);
    ...
    useCallback(() => {
      ...
      usersService.getMe()
        .then((me: any) => {
          ...
          if (typeof p.messageNotifications === 'boolean') setMessageNotifications(p.messageNotifications);
          if (typeof p.recruiterActivity === 'boolean') setRecruiterActivity(p.recruiterActivity);
          if (typeof p.profileVisible === 'boolean') setProfileVisible(p.profileVisible);
  ...
  const persist = useCallback((delta: Record<string, boolean>) => {
    const next = {
      matchAlerts, messageNotifications, recruiterActivity,
      profileVisible, showDistance,
      ...delta,
    };
    usersService.updateMe({ preferences: next }).catch(() => {});
  }, [matchAlerts, messageNotifications, recruiterActivity, profileVisible, showDistance]);

  const onMatchAlerts = (v: boolean) => { setMatchAlerts(v); persist({ matchAlerts: v }); };
  const onMessageNotifications = (v: boolean) => { setMessageNotifications(v); persist({ messageNotifications: v }); };
  const onRecruiterActivity = (v: boolean) => { setRecruiterActivity(v); persist({ recruiterActivity: v }); };
  const onProfileVisible = (v: boolean) => { setProfileVisible(v); persist({ profileVisible: v }); };
  const onShowDistance = (v: boolean) => { setShowDistance(v); persist({ showDistance: v }); };
